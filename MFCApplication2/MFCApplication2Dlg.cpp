...
void CMFCApplication2Dlg::OnBnClickedBtnCreate()
{
	...
	int r = _wtoi(mText.GetBuffer());	// 원의 반지름 가져오기
	mText.ReleaseBuffer();

	CRect rect;
	GetClientRect(&rect);

	int centerX = rand() % rect.Width();	// 원의 중심 설정 (다이얼로그 rand() 값)
	int centerY = rand() % rect.Height();
	int radius = r;

	dc.FillSolidRect(&rect, RGB(255, 255, 255));	// 화면 초기화 (#FFFFFF/흰색으로 본문 초기화)

	int penWidth = 5;
	CPen blackPen(PS_SOLID, penWidth, RGB(0, 0, 0));	// 검은색 펜으로 그리기
	...
	CPen yellowPen(PS_SOLID, penWidth, RGB(255, 255, 0));	// 노란색 펜으로 그리기
	dc.SelectObject(&yellowPen);
	dc.Ellipse(centerX - radius, centerY - radius, centerX + radius, centerY + radius);

	int crossSize = 5;
	CPen blackPen2(PS_SOLID, penWidth, RGB(0, 0, 0));	// 검은색 펜으로 변경
